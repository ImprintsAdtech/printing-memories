...
$(document).on('click','#SubmitButton',function(e){
	...
	//jQuery("#cancelUploading").show();
	jQuery("#cancelUploading").on("click", function(){
		location.reload();
	});
	var ImageFile = "";
...
$(document).on('click','.autofill_photo',function(){
	...
	$.ajax({
		...
		success: function(responseText) {
			if(responseText !='' && responseText == 1){
				location.reload();
			} else if(responseText == 'not allow'){
				$('.preloader').css('display','none');
				alert("You have already uploaded max photoes !");
			}
		}
	});
...
$(document).on('click','.deletePhotos',function(){
		...
		jQuery("#proFinishing").on("change", function(){
			jQuery(".preloader").css("display","block");
			var finish = jQuery(this).val();
			var freePhotoOrderId = jQuery("#freePhotoOrderId").val();
			jQuery.ajax({
				url: "<?=DEFAULT_URL?>/freeupload-photo-ajax.php?action=changeSetting",
				type:"POST",
				data:{finish:finish, freePhotoOrderId:freePhotoOrderId},
...
<script>
	jQuery('#file-1').on('change', function(){
		var allowed = jQuery("#no_of_photo").val();
		var uploadedPhotos = jQuery("#uploadedPhotos").val();
		var number = this.files.length;
		var totalPhotos = parseInt(uploadedPhotos) + parseInt(number);
		if(number != 0){
			if(totalPhotos<=allowed){
				/*var filesData ;
				for(var i=0; i<= number ; i++){
					filesData += JSON.stringify(this.files);
				}
				alert(filesData);
				console.log(filesData);*/
				//$("#puraniFile").JSON.stringify(this.files);
				jQuery("#numOfPhotsSelected").html(number +" Photos Selected");	
				jQuery('#SubmitButton').prop('disabled', false);	
			} else {
				jQuery('#SubmitButton').prop('disabled', true);	
				jQuery("#numOfPhotsSelected").html("Choose a file&hellip;");	
				alert("You have selected more then " +allowed+" photos !");
			}
		} else {
			alert("Kindly select photo");
			jQuery('#SubmitButton').prop('disabled', true);	
			//var puraniFile = jQuery.parseJSON(jQuery("#puraniFile").val());
			//files = puraniFile.files;
			//handleFiles(files);
		} 	
    });
	...
	$(document).ready(function () {
		...
		fbphotoSelect = function(id) {
			...
			callbackSubmit = function(photoId) {
				...
				var printOrderId = jQuery("#freePhotoOrderId").val();
				var productId = jQuery("#productId1").val();
				var guestUserId = jQuery("#guestUserId").val();
				var UserId = jQuery("#UserId").val();
				var productName = jQuery("#productName").val();
				var formData = {"photo_url":photo.source, 'freePhotoOrderId':printOrderId, 'productId':productId, 'userId':UserId, 'guestUserId':guestUserId, 'productName':productName};
				jQuery.ajax({
					url: "<?=DEFAULT_URL?>/freeupload-photo-ajax.php?action=imageFromFacebook",
					type: "POST",
					data:  formData,
			...
			FB.login(function (response) {
				if (response.authResponse) {
					...
					FB.api("/me", {fields : fields}, function(response){
						...
						/*
						var data = {'userFname':response.first_name, 'userLname':response.last_name, 'userGender':response.gender,'facebook_id':userId, 'userEmail':response.email, 'from':"facebook"};
						$.ajax({
		...
		*/
		$(".photoSelect").click(function (e) {
			e.preventDefault();
			jQuery('#uploadphoto').modal('toggle');
			fbphotoSelect();
		});
