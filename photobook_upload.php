...
?>
	...
	<title>Printmysnap - Free Photos for Lifetime | Upload Snapbook Photos</title>
	...
	<div class="col-md-12">
        ...
        <div class="col-md-3">
         	...
         	<span id="printPriceval"><b><?php if($printPrice == ""){echo $printPrice_arr[0];} else{ echo $printPrice;}?></b></span>
        ...
        <div class="col-md-3">
            ...
            <span id="nophotoval"><b><?php if($no_of_photo == "" || $no_of_photo == '0'){echo $no_of_photo_arr[0];} else{ echo $no_of_photo;}?></b></span>
...
<div class="clearfix">&nbsp;</div>
	...
	<div class="col-md-4 text-right">
		...
		$photosUploaded = $obj_product->getAllPremiumPrintPhoto($premiumPhotoOrderId, $userId);
	...
	<div class="four_sixphotos ars_CollMainbox">
		...
		if($db->numRows($photosUploaded)>0){
			...
			$query = "Select * from premiumPrintPhoto where premiumPhotoOrderId = ".$premiumPhotoOrderId." ORDER BY PremiumPhotoId ASC";

			$photosUploaded = mysql_query($query);

			while($photosUploadedData = mysql_fetch_object($photosUploaded)){

				$data[] = $photosUploadedData;
			...
			$releventData = array_chunk($data, $a);

			foreach($releventData as $releventData){

				foreach($releventData as $photosUploadedData){

					if($i == 1){

					?>

						<div class="col-md-6 text-center showoption ars_Colldiv<?php echo $d;?>" <?php echo "style='margin: 0 auto;float: none;'"; ?>>
					...
					?>
						...
						<div class="col-md-3 text-center showoption ars_Colldiv<?php echo $d;?>">
					...
					?>	
						...
						<img src="<?=DEFAULT_URL?>/uploads/<?=$photosUploadedData->premiumPhotoName?>" alt="Image<?php echo $d;?>" class="dragimg img-thumbnail printingPhoto">
						...
						<div class="showoptiondiv">
							...
							<ul>
							    ...
							    <li><a href="<?=DEFAULT_URL?>/editPhoto.php?photo=<?php echo base64_encode($photosUploadedData->PremiumPhotoId); ?>" target="_blank" class="croppingBtn"><i class="fa fa-crop"></i> EDIT</a></li>
	...
	<div class="tab-pane fade" id="cp_tab2">
      ...
      <li style="padding-top:20px;"><a href="javascript:void(0)" class="photoSelect" style="pointer-events:none;">Select Your Facebook Photos</a><!-- (You'll need to log in first)--></li>
	...
	<form action="" method="post" enctype="multipart/form-data" id="photobookupload">
	...
	<tr>
		...
		<td><input type="button"  id="SubmitButton" class="read-more" value="Upload" disabled/></td>
...
position: absolute;
    ...
    cursor: pointer;	
	 ...
	 <?php
		...
		<script src="<?=DEFAULT_URL?>/js/js/vendor/requirejs/require.js" data-main="<?=DEFAULT_URL?>/js/js/collection-home"></script>



		<?php

		include('photobook-upload-script.php');
