...
content sending to premiumPhotoOrder 



And saving that data to the same
...
$("#file-1").change(function (e) {
    ...
    if ((file = this.files)) {
    	...
    	for (var i = 0; i < file.length; ++i){
	        ...
	        img.src = _URL.createObjectURL(file[i]);
		...
		var ImageFile = file.name;
	  	...
	  	/*if(ImageFile == "") {
			...
			$.ajax({
				...
				url: "<?=DEFAULT_URL?>/premiumUpload-photo-ajax.php?action=printingPhoto",
		...
		$(document).on('click','.modal-delete',function(){
			...
			var premiumPhotoOrderId = $('#premiumOrderIdss').val();
				...
				$.ajax({
						...
						url: "<?=DEFAULT_URL?>/premiumUpload-photo-ajax.php",
	...
	$('.preloader').css('display','block');
		...
		if(blockimg_large != ""){			 
		  	...
		  	jQuery.ajax({
				...
				data : {blockimg_large:blockimg_large,printPhotoId:printPhotoId,productType:"premium"},
...
changing price of photo on size change
