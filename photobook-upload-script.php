	...
	$.map($('#file-1').get(0).files, function(file) {
		...
		var ImageFile = file.name;
	  	...
	  	if(ImageFile == "") {
			...
			var formData = new FormData($('form#photobookupload')[0]);



			$.ajax({

				url: "<?=DEFAULT_URL?>/photobook-upload-ajax.php?action=printingPhoto",
		...
		$(document).on('click','.modal-delete',function(){
			...
			var premiumPhotoOrderId = $('#premiumOrderIdss').val();
				...
				$.ajax({
						...
						url: "<?=DEFAULT_URL?>/photobook-upload-ajax.php",
					...
					// $('#preview').attr('src',blockimg_large);
					 ...
					 //alert(blockimg_large);	
						...
						{			 
							  ...
							  data : {blockimg_large:blockimg_large,printPhotoId:printPhotoId,productType:"photobook"},
...
$(document).ready(function () {
				...
				FB.api("/me", {fields : fields}, function(response){
					...
					$.ajax({
						...
						success: function(response){
							...
							jQuery(".photoSelect").addClass("read-more");
	...
	$(".photoSelect").click(function (e) {
		...
		jQuery("#uploadphoto").removeClass("in");

		//jQuery("#uploadphoto").css("display","none");
